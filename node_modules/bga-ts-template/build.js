...
$machinestates = ${writer.stringify(statesJSON, true)};`
		...
		fs.writeFileSync('devilsdice.action.php',
`<?php
${writer.fileSignature}
...
if (fs.existsSync('source/client/tsconfig.json')) {
	...
	const tsCommand = 'npx tsc -p source/client --outFile devilsdice.js' + (builder.watch ? ' -w' : '');
	builder.execCommand(`Typescript compiler: client/define.ts => devilsdice.js`, tsCommand);

	// const cookbookModuleRegex = /"@cookbook\/([^"]+)"/g;
	// const searchString = 'define("devilsdice", [';
	// const importFixer = () => {
	// 	if (!fs.existsSync('devilsdice.js'))
	// 		return;

	// 	const data = fs.readFileSync('devilsdice.js', 'utf8');
	// 	const start = data.indexOf(searchString);
	// 	if (start !== -1) {
	// 		const end = start + searchString.length;
	// 		const result = data.substring(0, start) + 'define([' + data.substring(end);
	// 		fs.writeFileSync('devilsdice.js', result, 'utf8');
	...
	// builder.watchCommand("Import Generator: source/client/build/imports.d.ts", 'devilsdice.js', importFixer);
}

if (fs.existsSync('source/client/devilsdice.scss')) {
	builder.execCommand(`SCSS compiler: client/devilsdice.scss => devilsdice.css`, `npx sass --no-source-map source/client/devilsdice.scss devilsdice.css` + (builder.watch ? ' --watch' : ''));
}
